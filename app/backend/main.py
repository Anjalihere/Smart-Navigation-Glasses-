...
from openai import APIError, AzureOpenAI, NotFoundError
...
async def speech_to_text(audio: UploadFile = File(...)) -> Dict[str, Any]:
    return {
        "transcript": "Speech-to-text route is scaffolded. Wire Azure Speech SDK next.",
        "filename": audio.filename,
    }


@app.post("/speech")
async def speech(text: str = Form(...)) -> Dict[str, Any]:
    return {
        "audioUrl": "",
        "text": text,
        "message": "Text-to-speech route is scaffolded. Wire Azure Speech TTS next.",
    }
